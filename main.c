...
static uint32_t parse_color(const char *color) {
	...
	if (len != 6 && len != 8) {
		wakeman_log(LOG_DEBUG, "Invalid color %s, defaulting to 0xFFFFFFFF",
				color);
		return 0xFFFFFFFF;
	}
...
struct wakeman_state {
	struct wl_display *display;
	struct wl_compositor *compositor;
	struct wl_shm *shm;
	struct zwlr_layer_shell_v1 *layer_shell;
	struct zxdg_output_manager_v1 *xdg_output_manager;
	struct wl_list configs;  // struct wakeman_output_config::link
	struct wl_list outputs;  // struct wakeman_output::link
	bool run_display;
};

struct wakeman_output_config {
	char *output;
...
struct wakeman_output {
	...
	struct wakeman_state *state;
	struct wakeman_output_config *config;
...
bool is_valid_color(const char *color) {
	int len = strlen(color);
	if (len != 7 || color[0] != '#') {
		wakeman_log(LOG_ERROR, "%s is not a valid color for wakeman. "
...
static void render_frame(struct wakeman_output *output) {
...
static void destroy_wakeman_output_config(struct wakeman_output_config *config) {
	if (!config) {
		return;
	}
...
static void destroy_wakeman_output(struct wakeman_output *output) {
	if (!output) {
		return;
	}
...
static void layer_surface_configure(void *data,
	...
	struct wakeman_output *output = data;
	output->width = width;
...
static void layer_surface_closed(void *data,
		struct zwlr_layer_surface_v1 *surface) {
	struct wakeman_output *output = data;
	wakeman_log(LOG_DEBUG, "Destroying output %s (%s)",
			output->name, output->identifier);
	destroy_wakeman_output(output);
}
...
static void find_config(struct wakeman_output *output, const char *name) {
	struct wakeman_output_config *config = NULL;
...
static void xdg_output_handle_description(void *data,
	...
	if (paren) {
		...
		if (!output->identifier) {
			wakeman_log(LOG_ERROR, "Failed to allocate output identifier");
			return;
		}
...
static void create_layer_surface(struct wakeman_output *output) {
...
static void xdg_output_handle_done(void *data,
	...
	struct wakeman_output *output = data;
	if (!output->config) {
		wakeman_log(LOG_DEBUG, "Could not find config for output %s (%s)",
				output->name, output->identifier);
		destroy_wakeman_output(output);
	} else if (!output->layer_surface) {
		wakeman_log(LOG_DEBUG, "Found config %s for output %s (%s)",
...
static void handle_global(void *data, struct wl_registry *registry,
	...
	struct wakeman_state *state = data;
	if (strcmp(interface, wl_compositor_interface.name) == 0) {
		...
		struct wakeman_output *output = calloc(1, sizeof(struct wakeman_output));
		output->state = state;
		output->wl_name = name;
		output->wl_output =
...
static void handle_global_remove(void *data, struct wl_registry *registry,
		uint32_t name) {
	struct wakeman_state *state = data;
	struct wakeman_output *output, *tmp;
...
static bool store_wakeman_output_config(struct wakeman_state *state,
		struct wakeman_output_config *config) {
	struct wakeman_output_config *oc = NULL;
...
static void parse_command_line(int argc, char **argv,
	...
	struct wakeman_output_config *config = calloc(sizeof(struct wakeman_output_config), 1);
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "c:hi:I:m:o:v", long_options, &option_index);
		if (c == -1) {
			break;
		}
		switch (c) {
		case 'c':  // color
			if (!is_valid_color(optarg)) {
				wakeman_log(LOG_ERROR, "Invalid color: %s", optarg);
				continue;
			}
		...
		case 'i':  // image
			...
			if (!config->image) {
				wakeman_log(LOG_ERROR, "Failed to load image: %s", optarg);
			}
			break;
		case 'I':  // video
			free(config->image);
			config->image = load_background_video(optarg);
			if (!config->image) {
				wakeman_log(LOG_ERROR, "Failed to load video: %s", optarg);
			}
			break;
		case 'm':  // mode
			config->mode = parse_background_mode(optarg);
			if (config->mode == BACKGROUND_MODE_INVALID) {
				wakeman_log(LOG_ERROR, "Invalid mode: %s", optarg);
			}
			break;
		case 'o':  // output
			if (config && !store_wakeman_output_config(state, config)) {
				// Empty config or merged on top of an existing one
				destroy_wakeman_output_config(config);
			}
			config = calloc(sizeof(struct wakeman_output_config), 1);
			config->output = strdup(optarg);
			config->mode = BACKGROUND_MODE_INVALID;
			wl_list_init(&config->link);  // init for safe removal
			break;
		case 'v':  // version
			fprintf(stdout, "wakeman version " WAKEMAN_VERSION "\n");
			exit(EXIT_SUCCESS);
			break;
		default:
	...
	if (optind < argc) {
		config = NULL;
		struct wakeman_output_config *tmp = NULL;
...
int main(int argc, char **argv) {
	...
	if (!state.display) {
		wakeman_log(LOG_ERROR, "Unable to connect to the compositor. "
	...
	if (state.compositor == NULL || state.shm == NULL ||
		...
		wakeman_log(LOG_ERROR, "Missing a required Wayland interface");
		return 1;
	}

	struct wakeman_output *output;
	...
	struct wakeman_output *tmp_output;
	wl_list_for_each_safe(output, tmp_output, &state.outputs, link) {
		destroy_wakeman_output(output);
	}

	struct wakeman_output_config *config = NULL, *tmp_config = NULL;
